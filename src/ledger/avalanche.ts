...
const { ParseablePlatformEnum } = TxHelper;
...
export class OmniAvalancheLedgerWallet extends PublicMnemonicWallet {
  ...
  static async fromTransport(
    ...
    const config = await getLedgerConfigAvax(avalancheTransport);
  ...
  >(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = 0; j < addrs.length; j++) {
        ...
        const pathStr = await this.getPathFromAddress(srcAddr); // returns change/index
        paths.push(pathStr);
      }
    }
  ...
  >(
    ...
    const ledgerSignedTx = await appAvax.signTransaction(
      accountPath,
      bip32Paths,
      txbuff,
    );
  ...
  >(
    ...
    const sigMap = await appAvax.signHash(accountPath, bip32Paths, msg);
